	...
	<div>
		{#if Object.keys(tableData).length > 0}
			{#each Object.entries(tableData).sort( ([subA, _A], [subB, _B]) => subA.localeCompare(subB) ) as [subject, courses]}
				<h3 class="subject">{subject} {"\u2013"} {subjectNames[subject]}</h3>
					...
					{#each Object.entries(courses).sort( ([courseNumA, _A], [courseNumB, _B]) => courseNumA.localeCompare(courseNumB) ) as [courseNumber, sections]}
						<tr>
							<th class="course-details" rowspan="2">
								<h3>{courseNumber}</h3>
								<span class="tooltip">{courseNames[subject + courseNumber]}</span>
							</th>
		...
		.course-details {
			...
			.tooltip {
				...
				&::before {
					...
					position: absolute;
					left: 50%;
					transform: translate(-50%, calc(-1 * var(--padding-top) + 2px)) rotate(45deg);
					background-color: var(--bg);
					padding: 4px;
					z-index: 1;
				}
